...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var leDate = Date.now();
		if (leDate.getHours() <= 9) {
			var achieveID:Int = Achievements.getAchievementIndex('rageindamorn');
			if(!Achievements.isAchievementUnlocked(Achievements.achievementsStuff[achieveID][2])) { // Have some orange juice.
	...
	function giveAchievement() {
		add(new AchievementObject('rageindamorn', camAchievement));
		FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
		trace('Giving achievement');
	}
	#end
